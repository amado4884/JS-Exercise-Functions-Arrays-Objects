...
function getModelYears(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    ...
    response.push(element.car_year);
  }
  return response;
}
...
function getCarInfoById(inventory, id) {
  for (let i = 0; i < inventory.length; i++) {
    ...
    if (item.id === id) return `This is a ${item.car_make} ${item.car_model}`;
  }
}
...
function getOlderCars(inventory, maxYear) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    ...
    if (car.car_year <= maxYear) response.push(car);
  }
  return response;
}
